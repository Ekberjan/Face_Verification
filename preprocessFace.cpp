...
void preprocessFace::equalizeLeftAndRightHalves(cv::Mat &faceImg)
{
...
bool preprocessFace::writeResults(cv::Mat &inputImage, cv::Mat &inputFace, cv::Mat &processedFace)
{
	try
	{
		cv::imwrite(inputImageDirName + std::to_string(inputFaceCount) + ".png", inputImage);
		cv::imwrite(inputFaceDirName + std::to_string(inputFaceCount) + ".png", inputFace);
		cv::imwrite(outputFaceDirName + std::to_string(inputFaceCount) + ".png", processedFace);

		cv::Rect upperFace(0, 0, processedFace.rows, processedFace.cols / 3);
		cv::Rect middleFace(0, processedFace.cols / 3, processedFace.rows, processedFace.cols / 3);
		cv::Rect bottomFace(0, 2 * processedFace.cols / 3, processedFace.rows, processedFace.cols / 3);

		cv::Mat eyeRegion = processedFace(upperFace);
		cv::Mat noseRegion = processedFace(middleFace);
		cv::Mat mouthRegion = processedFace(bottomFace);

		cv::resize(eyeRegion, eyeRegion, outputImgSize);
		cv::resize(noseRegion, noseRegion, outputImgSize);
		cv::resize(mouthRegion, mouthRegion, outputImgSize);

		cv::imwrite(outputEyeDirName + std::to_string(inputFaceCount) + ".png", eyeRegion);
		cv::imwrite(outputNoseDirName + std::to_string(inputFaceCount) + ".png", noseRegion);
		cv::imwrite(outputMouthDirName + std::to_string(inputFaceCount) + ".png", mouthRegion); 

		if (eyeRegion.cols > 0 && eyeRegion.rows > 0)
		{
			cv::imshow("eye region", eyeRegion);
		}

		if (noseRegion.cols > 0 && noseRegion.rows > 0)
		{
			cv::imshow("nose region", noseRegion);
		}

		if (mouthRegion.cols > 0 && mouthRegion.rows > 0)
		{
			cv::imshow("mouth region", mouthRegion);
		}

	}
	catch (std::string error)
	{
		std::cout << "Error happened in preprocessFace::writeResults() function. " << std::endl;
