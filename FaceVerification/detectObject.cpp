...
void detectObject::detectObjectsCustom(const cv::Mat &img, cv::CascadeClassifier &cascade, cv::vector<cv::Rect> &objects, int scaledWidth, int flags, cv::Size minFeatureSize, float searchScaleFactor, int minNeighbors)
{
	...
	if (img.cols > scaledWidth) {
		...
		resize(gray, inputImg, cv::Size(scaledWidth, scaledHeight));
	}
	else {
	...
	//cascade.detectMultiScale(inputImg, objects, searchScaleFactor, minNeighbors, flags, minFeatureSize);
...
void detectObject::detectLargestObject(const cv::Mat &img, cv::CascadeClassifier &cascade, cv::Rect &largestObject, int scaledWidth)
{
	// Only search for just 1 object (the biggest in the image).
	int flags = cv::CASCADE_FIND_BIGGEST_OBJECT;// | CASCADE_DO_ROUGH_SEARCH;
...
// Input is temporarily shrunk to 'scaledWidth' for much faster detection, since 200 is enough to find faces.
// Note: detectLargestObject() should be faster than detectManyObjects().
void detectObject::detectManyObjects(const cv::Mat &img, cv::CascadeClassifier &cascade, cv::vector<cv::Rect> &objects, int scaledWidth)
{
